...
case "$(val macs)" in
  *sha1*|*umac-64*) W "sshd still offers sha1 or 64-bit-tag MACs" "pin MACs to sha2-etm/umac-128-etm (ssh-crypto step)" ;;
  *)                P "MACs are etm-only, 128-bit tags or better (no sha1)" ;;
esac
case "$(val kexalgorithms)" in
  *sha1*|*ecdh-sha2-nistp*) W "key exchange still offers NIST P-curves or sha1" "pin KexAlgorithms to the PQ hybrids + curve25519 (ssh-crypto step)" ;;
  *mlkem768*|*sntrup761*)   P "key exchange is PQ-hybrid + curve25519 only" ;;
  *)                        W "key exchange has no post-quantum hybrid" "pin KexAlgorithms (ssh-crypto step)" ;;
esac
case "$(val ciphers)" in
  *cbc*) F "sshd still offers CBC-mode ciphers" "pin Ciphers to AEAD + ctr (ssh-crypto step)" ;;
...
[ "$(val allowtcpforwarding)" = no ] \
  ...
  || W "TCP forwarding enabled" "add 'AllowTcpForwarding no' (ssh policies step)"
...
for opt in nodev nosuid noexec; do
  case ",$shm_opts," in
    ...
    *) W "/dev/shm is missing $opt" "remount /dev/shm with $opt (mount options step)";;
  esac
done
...
for opt in nodev nosuid noexec; do
  case ",$tmp_opts," in
    ...
    *) W "/tmp is missing $opt" "run harden.sh (tmp-confinement step)";;
  esac
done
...
# Purged by the step: the three daemons a server runs because a package
...
for pkg in avahi-daemon cups-daemon rpcbind; do
  if on_node dpkg -s "$pkg" >/dev/null 2>&1; then
    F "$pkg is installed (a network listener nobody asked for)" "run harden.sh (service-purge step) or apt-get purge $pkg"
  else
...
if [ -z "$iou" ]; then
  ...
  W "kernel.io_uring_disabled is $iou, not 2" "run harden.sh (kernel-surface step)"
fi
for kv in kernel.sysrq:0 dev.tty.ldisc_autoload:0; do
  ...
  if [ "$got" = "$want" ]; then
    ...
    W "$key is ${got:-unreadable}, not $want" "run harden.sh (kernel-surface step)"
  fi
done
...
if [ -z "$uns" ]; then
  ...
  W "kernel.unprivileged_userns_clone is $uns, not 0" "run harden.sh (kernel-surface step)"
fi
...
[ -z "$diet_bad" ] \
  ...
  || W "setuid/setgid still on:$diet_bad" "run harden.sh (suid-diet step): dpkg-statoverride --update --add root root 0755 <file>"
...
if on_node grep -qE '^\*\s+hard\s+nproc\s+[0-9]+' /etc/security/limits.d/99-hardening-nproc.conf 2>/dev/null; then
  ...
  W "login sessions have no process cap - a fork bomb from any account takes the box down" "run harden.sh (process-limits step): '* hard nproc 4096' in limits.d"
fi
missing_pl=""
...
if [ "$(on_node systemctl is-enabled ctrl-alt-del.target 2>/dev/null)" = "masked" ]; then
  ...
  W "ctrl-alt-del.target is $(on_node systemctl is-enabled ctrl-alt-del.target 2>/dev/null) - a console Ctrl+Alt+Del reboots the box" "run harden.sh (console-reboot step): systemctl mask ctrl-alt-del.target"
fi
if on_node grep -qxF 'CtrlAltDelBurstAction=none' /etc/systemd/system.conf.d/99-hardening-ctrlaltdel.conf 2>/dev/null; then
  ...
  W "the Ctrl+Alt+Del burst action is at its default (reboot-force) - seven presses in 2 s reboot hard" "run harden.sh (console-reboot step)"
fi
...
if on_node grep -Eq '^[^#]*pam_unix\.so.*[[:space:]]nullok([[:space:]]|$)' /etc/pam.d/common-auth 2>/dev/null; then
  F "pam_unix in common-auth has nullok - an account with an empty password logs in with no password" "run harden.sh (pam-nullok step)"
else
...
if on_node ufw status verbose | grep -qE '(reject|deny) \(outgoing\)'; then
  ...
  W "outbound traffic is unrestricted - a reverse shell, a dropper or an exfil leaves on any port" "run harden.sh (egress step), or: ufw default reject outgoing"
fi
...
if [ -z "$missing_out" ] || on_node ufw status verbose | grep -q 'allow (outgoing)'; then
  ...
  W "the outbound policy allows no:$missing_out - resolution, the clock, updates or the DHCP lease will fail" "ufw allow out <port> for each, or pass --egress-allow"
fi
...
case "$v" in
  ...
  *)       F "Unsigned repositories are ACCEPTED (AllowInsecureRepositories=$v)" "run harden.sh (apt-trust step)";;
esac
v=$(apt_eff APT::Get::AllowUnauthenticated)
case "$v" in
  false|0) P "Unauthenticated installs are refused (AllowUnauthenticated=$v)";;
  "")      W "AllowUnauthenticated is not pinned (default false, one stray conf line from true)" "pin it in apt.conf.d (apt-trust step)";;
  *)       F "Unauthenticated installs are ACCEPTED (AllowUnauthenticated=$v)" "run harden.sh (apt-trust step)";;
esac
v=$(apt_eff Acquire::Check-Valid-Until)
case "$v" in
  true|1) P "Expired Release files are refused (Check-Valid-Until pinned)";;
  *)      W "Check-Valid-Until is not pinned (default true; a freeze attack needs it off)" "pin it in apt.conf.d (apt-trust step)";;
esac
...
[ "$(val banner)" != "none" ] && [ -n "$(val banner)" ] \
  ...
  || W "sshd has no pre-auth banner" "set Banner /etc/issue.net (banners step)"
...
on_node grep -rqE '^Defaults\s+use_pty' /etc/sudoers /etc/sudoers.d \
  ...
  || W "use_pty not set" "add 'Defaults use_pty' (sudo hardening step)"
...
[ "$sudoers_perm" = "440" ] \
  ...
  || W "sudo drop-in is ${sudoers_perm:-absent}" "install -m 440 (sudo hardening step)"
...
on_node grep -rqE '^\*[[:space:]]+hard[[:space:]]+core[[:space:]]+0' /etc/security/limits.conf /etc/security/limits.d \
  ...
  || W "no '* hard core 0' limit" "add a limits.d drop-in (core dump step)"
on_node grep -rqE '^root[[:space:]]+hard[[:space:]]+core[[:space:]]+0' /etc/security/limits.conf /etc/security/limits.d \
  ...
  || W "root can still dump core" "add 'root hard core 0' (core dump step)"
on_node grep -rqsE '^Storage=none' /etc/systemd/coredump.conf /etc/systemd/coredump.conf.d \
  ...
  || W "systemd-coredump would still store dumps" "set Storage=none (core dump step)"
on_node grep -rqsE '^ProcessSizeMax=0' /etc/systemd/coredump.conf /etc/systemd/coredump.conf.d \
  ...
  || W "systemd-coredump would still process dumps" "set ProcessSizeMax=0 (core dump step)"
...
[ "$umask_defs" = "027" ] || [ "$umask_defs" = "077" ] \
  ...
  || W "login.defs UMASK is ${umask_defs:-unset}" "set UMASK 027 (umask & timeout step)"
on_node grep -rqsE '^umask[[:space:]]+0?27' /etc/profile.d \
  && P "profile.d sets umask 027 for login shells" \
  || W "no profile.d umask drop-in" "add umask 027 (umask & timeout step)"
...
[ -n "$tmout_val" ] && [ "$tmout_val" -le 900 ] 2>/dev/null \
  ...
  || W "no readonly TMOUT in profile.d" "add readonly TMOUT=900 (umask & timeout step)"
on_node grep -rqsE '^export TMOUT' /etc/profile.d \
  && P "TMOUT is exported to the session" \
  || W "TMOUT not exported" "add export TMOUT (umask & timeout step)"
...
[ "$crontab_perm" = "600 root root" ] \
  ...
  || W "/etc/crontab is ${crontab_perm:-missing}" "chmod 600, chown root:root (cron step)"
...
[ "$cron_dirs" = "700 root root" ] \
  ...
  || W "cron dirs are not uniformly 700 root:root" "chmod 700, chown root:root (cron step)"
if on_node test -f /etc/cron.allow && [ "$(on_node stat -c '%a %U %G' /etc/cron.allow)" = "640 root root" ]; then
  ...
  W "no root-only cron.allow" "write cron.allow with just root, 640 (cron step)"
fi
...
on_node test -x /usr/bin/rkhunter \
  ...
  || W "rkhunter not installed" "apt install rkhunter (rkhunter step)"
on_node test -s /var/lib/rkhunter/db/rkhunter.dat \
  && P "rkhunter property baseline present" \
  || W "no rkhunter property baseline" "run rkhunter --propupd (rkhunter step)"
[ "$(on_node systemctl is-enabled rkhunter-check.timer 2>/dev/null)" = enabled ] \
  && P "Daily rkhunter check timer enabled" \
  || W "no daily rkhunter check timer" "enable rkhunter-check.timer (rkhunter step)"
...
on_node test -f /etc/modprobe.d/99-hardening-blacklist.conf \
  ...
  || W "no modprobe blacklist drop-in" "run the module-blacklist step (CIS 1.1.1/3.4)"
...
on_node modprobe -n -v dccp 2>/dev/null | grep -q 'install /bin/false' \
  ...
  || W "modprobe does not defeat dccp" "check the install directive (module-blacklist step)"
...
on_node grep -qE '^deny[[:space:]]*=[[:space:]]*[1-5]$' /etc/security/faillock.conf \
  ...
  || W "no faillock deny threshold <= 5" "set deny = 5 in /etc/security/faillock.conf (faillock step)"
on_node grep -qE '^unlock_time[[:space:]]*=[[:space:]]*[0-9]+' /etc/security/faillock.conf \
  ...
  || W "no faillock unlock_time" "set unlock_time = 900 (faillock step)"
...
on_node sshd -T 2>/dev/null | grep -qi '^allowgroups ' \
  ...
  || W "sshd has no AllowGroups restriction" "limit SSH login to a group (ssh-access step)"
...
on_node systemctl show fail2ban -p NoNewPrivileges 2>/dev/null | grep -q '^NoNewPrivileges=yes$' \
  ...
  || W "fail2ban has no NoNewPrivileges" "add a systemd sandboxing drop-in (service-sandboxing step)"
on_node systemctl show fail2ban -p ProtectSystem 2>/dev/null | grep -qE '^ProtectSystem=(full|strict)$' \
  ...
  || W "fail2ban filesystem not protected" "set ProtectSystem=full (service-sandboxing step)"
...
on_node bash -c "systemd-analyze cat-config systemd/journald.conf 2>/dev/null | grep -qE '^Storage=persistent'" \
  ...
  || W "journald logs are volatile" "set Storage=persistent (journald step)"
on_node test -d /var/log/journal \
  && P "the persistent journal directory exists" \
  || W "no /var/log/journal" "create it / set Storage=persistent (journald step)"
...
on_node grep -Eq '^auth[[:space:]]+required[[:space:]]+pam_wheel\.so.*use_uid' /etc/pam.d/su \
  ...
  || W "su is open to anyone with the password" "gate su on a group with pam_wheel (su restriction step)"
...
[ -z "$(printf '%s' "$leakylogs" | tr -d ' ')" ] \
  ...
  || W "log files readable/writable beyond owner+group ($leakylogs)" "chmod g-wx,o-rwx (log-permissions step)"
...
on_node apt-config dump | grep -qF 'chmod g-wx,o-rwx /var/log/apt/history.log' \
  ...
  || W "apt resets history.log/eipp.log.xz to 644 on its next run - nothing re-tightens them" "write the apt hook (log-permissions step)"
...
if on_node bash -c 'command -v rsyslogd' >/dev/null 2>&1; then
  on_node grep -qs '^\$FileCreateMode 0640' /etc/rsyslog.d/99-hardening.conf \
    ...
    || W "rsyslog file-creation mode is not pinned" "write the drop-in (log-permissions step)"
else
...
if on_node bash -c 'command -v logrotate' >/dev/null 2>&1; then
  on_node grep -Eqs '^create 0640' /etc/logrotate.conf \
    ...
    || W "logrotate global create is bare or loose (rotated logs clone or loosen)" "pin 'create 0640' in logrotate.conf (logrotate-perms step)"
  ...
  [ -z "$(printf '%s' "$loosecreate" | tr -d ' ')" ] \
    ...
    || W "logrotate.d snippets re-create logs too open ($loosecreate)" "tighten their create modes (logrotate-perms step)"
else
...
on_node dpkg -s auditd >/dev/null \
  ...
  || F "auditd is not installed" "apt install auditd (auditd step)"
[ "$(on_node systemctl is-enabled auditd)" = enabled ] \
  && P "auditd is enabled at boot" \
  || F "auditd is not enabled at boot" "systemctl enable auditd (auditd step)"
...
[ -z "$(printf '%s' "$missingkeys" | tr -d ' ')" ] \
  ...
  || F "staged audit rules incomplete (missing: $missingkeys)" "re-run the auditd step"
[ "$(on_node stat -c '%a %U %G' /etc/audit/rules.d/hardening.rules)" = "640 root root" ] \
  ...
  || W "the staged audit ruleset permissions drifted" "chmod 0640 + chown root:root (auditd step)"
on_node grep -Eqs '^max_log_file_action = keep_logs' /etc/audit/auditd.conf \
  ...
  || W "auditd rotates its history away" "pin max_log_file_action = keep_logs (auditd step)"
...
[ -z "$(printf '%s' "$loose" | tr -d ' ')" ] \
  ...
  || W "interactive homes readable beyond owner+group ($loose)" "chmod g-w,o-rwx on each (home-permissions step)"
[ "$(on_node stat -c %a /root)" = "700" ] \
  && P "root's home keeps Debian's 700" \
  || W "root's home has drifted from 700" "chmod 700 /root (home-permissions step)"
...
[ -z "$(printf '%s' "$relics" | tr -d ' ')" ] \
  ...
  || F "passwordless-access relics present: $relics" "remove them (home-permissions step)"
...
[ -z "$(printf '%s' "$fwd" | tr -d ' ')" ] \
  ...
  || W ".forward files present: $fwd" "remove them (home-permissions step)"
...
on_node findmnt -no OPTIONS /proc | grep -Eq 'hidepid=(2|invisible)' \
  ...
  || W "/proc shows every user's processes (ps aux leaks command lines)" "remount /proc with hidepid=2 (process-isolation step)"
on_node grep -Eqs '^[^#[:space:]]+[[:space:]]+/proc[[:space:]].*hidepid=' /etc/fstab \
  && P "the hidepid option is pinned in fstab (survives a reboot)" \
  || W "hidepid is not pinned in fstab" "add a /proc line with hidepid (process-isolation step)"
[ "$(sctl kernel.yama.ptrace_scope)" = "1" ] \
  && P "ptrace is restricted to direct parents (yama.ptrace_scope=1)" \
  || W "any process can ptrace another of the same user" "set kernel.yama.ptrace_scope=1 (process-isolation step)"
...
[ "$ycf" = "11" ] \
  ...
  || W "yescrypt cost factor is ${ycf:-unset} (stock 5: milliseconds per guess)" "set YESCRYPT_COST_FACTOR 11 in login.defs (guess-cost step)"
fd=$(ld FAIL_DELAY)
if [ -n "$fd" ] && [ "$fd" -ge 4 ] 2>/dev/null; then
  P "failed logins wait ${fd} s before returning (online guessing throttled)"
else
  W "FAIL_DELAY is ${fd:-unset}" "set FAIL_DELAY 5 in login.defs (guess-cost step)"
fi
on_node grep -q pam_faildelay /etc/pam.d/common-auth \
  && P "pam_faildelay is wired into the auth stack" \
  || W "pam_faildelay not wired into common-auth" "enable the hardening-faildelay profile (guess-cost step)"
...
[ -z "$(printf '%s' "$sup_bad" | tr -d ' ')" ] \
  ...
  || W "ENV_SUPATH has unsafe entries: $sup_bad" "drop empty/relative/writable entries (root-path step)"
prof_bad=""
...
[ -z "$(printf '%s' "$prof_bad" | tr -d ' ')" ] \
  ...
  || F "/etc/profile PATH has unsafe entries: $prof_bad" "sanitize the PATH lines (root-path step)"
...
[ -z "$(printf '%s' "$cron_bad" | tr -d ' ')" ] \
  ...
  || W "/etc/crontab PATH has unsafe entries: $cron_bad" "sanitize PATH= in /etc/crontab (root-path step)"
...
on_node systemctl show apt-daily-upgrade.service -p NoNewPrivileges 2>/dev/null | grep -q '^NoNewPrivileges=yes$' \
  ...
  || W "apt updater has no NoNewPrivileges" "add a systemd sandboxing drop-in (apt-sandboxing step)"
...
[ -n "$remember" ] && [ "$remember" -ge 24 ] 2>/dev/null \
  ...
  || W "old passwords can be reused (remember=${remember:-unset})" "enable pam_pwhistory with remember=24 (pw-history step)"
...
on_node bash -c "grep -v '^#' /etc/pam.d/common-password | grep pam_pwhistory | grep -q enforce_for_root" 2>/dev/null \
  ...
  || W "root bypasses the history check" "add enforce_for_root to pam_pwhistory (pw-history step)"
on_node stat -c '%a %U %G' /etc/security/opasswd 2>/dev/null | grep -q '^600 root root$' \
  ...
  || W "opasswd is loose or missing" "install root:root 0600 /etc/security/opasswd (pw-history step)"
...
[ -z "$srv_found" ] \
  ...
  || F "Legacy protocol servers present:$srv_found" "purge them (harden.sh step 36)"
...
[ -z "$cli_found" ] \
  ...
  || W "Legacy cleartext clients present:$cli_found" "run harden.sh (legacy protocol purge)"
nis_st=$(on_node dpkg-query -W -f '${db:Status-Status}' nis) || true
case "$nis_st" in
  installed|config-files) F "NIS is installed" "apt-get purge nis (harden.sh step 36)";;
...
for kv in randomize_va_space:2 kexec_load_disabled:1 unprivileged_bpf_disabled:1 perf_event_paranoid:3; do
  ...
  if [ "$got" = "$want" ]; then
    ...
    W "$key is ${got:-unreadable}, not $want" "run harden.sh (exploit-mitigations step)"
  fi
done
...
if [ -z "$jit" ]; then
  ...
  W "net.core.bpf_jit_harden is $jit, not 2" "run harden.sh (exploit-mitigations step)"
fi
...
[ -z "$empty" ] \
  ...
  || F "Accounts with empty password: $empty" "lock or set passwords (account-hygiene step)"
...
if on_node grep -qE '^[+-]' /etc/passwd /etc/shadow /etc/group; then
  F "NIS compat ('+'/'-') entries present in the account database" "remove them (harden.sh account-hygiene step)"
else
...
[ -z "${unshadowed// /}" ] \
  ...
  || F "Password material in world-readable /etc/passwd: $unshadowed" "run pwconv (harden.sh account-hygiene step)"
