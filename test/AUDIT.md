...
The checks are grouped into one category per hardening step — the `--`
section headers in the audit output, from SSH authentication (core) down to
the closing accounts & files sweep. When `harden.sh` grows a step, the audit
grows a section.
...
root-only `cron.allow`, and `cron.deny` gone (allow-list model). Step 15
...
actually promises (the log-permissions sweep and the logrotate create-mode
pin are the two most recent). Current score on a freshly hardened node:

```
 Score: 95 PASS, 0 WARN, 0 FAIL  ->  100% compliant
```
