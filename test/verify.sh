...
# With LogLevel VERBOSE (step 11) every deliberately-failed probe below
...
# tests banning on purpose (the sibling Ansible repo's CI caught it; here it
# was a race with fail2ban's journal poll). Shield this client while the
# functional checks run; the shield is lifted right before the ban test.
# ignoreip means the probes are never counted, so it still starts from zero.
docker exec db-harden-node fail2ban-client set sshd addignoreip 172.17.0.1 >/dev/null 2>&1 || true
...
# Ask the kernel for the EFFECTIVE values — not the file we wrote.
...
expect_line "fstab pins /dev/shm with nodev,nosuid,noexec" \
  "nodev,nosuid,noexec" \
...
on_node cp /bin/true /dev/shm/db-harden-probe 2>/dev/null || true
if on_node /dev/shm/db-harden-probe >/dev/null 2>&1; then
...
on_node rm -f /dev/shm/db-harden-probe 2>/dev/null || true
...
# SIGPIPE — the pipeline then "fails" despite the match, exactly the trap
# the banner and journalctl checks above call out. A pure-bash case has no
# pipe to break.
case "$aide_out" in
...
# from red to green when the step runs.
...
# declared for every module the step promises, not just the install line.
...
expect_line "the shadow BACKUP (/etc/shadow-) got the same treatment" '^640 root shadow$' \
  "sudo stat -c '%a %U %G' /etc/shadow-"
# The e2e plants a world-writable file, an orphan (UID 12345) and a 777 dir
# BEFORE the first hardening pass — so these three sweeps genuinely flip
# from red to green when the step runs, instead of asserting a vacuum.
...
# The whole point: sandboxed AND still working. If the confinement had broken
# it, the unit wouldn't be active — and the ban test below would fail anyway.
...
# The CI plants `dhsvc` BEFORE hardening with /bin/bash and a real password —
# a packaged-daemon leftover, and a valid su / SSH target. Both gates must
# have been taken away by the step.
...
# refuses. A shell that opens would print the uid; nologin prints its refusal
# instead. This is the honest signal (the same reasoning as step 24: don't
# trust exit codes that several mechanisms share).
...
# The sweep must not have swallowed the humans or the recovery paths.
...
# The CI plants /var/log/dh-app.log at 0666 BEFORE hardening; the sweep must
...
# Sweep-wide: nothing under /var/log is world- or group-writable-accessible
...
# FileCreateMode to 0644 before hardening. Delete syslog, restart, log one
...
echo "== Step 27: logrotate re-creates logs restricted =="
...
# stock Debian, tightened by the step — must re-create dpkg.log 0640 too,
...
echo "== Step 28: auditd staged and enabled (CIS 4.1) =="
...
echo "== Step 29: home directory permissions (CIS 6.2) =="
# The CI plants dhhome with a 755 home plus a .netrc and a .forward before
# the first pass — these checks flip from red to green because the sweep did
...
echo "== Step 30: process isolation (/proc hidepid + ptrace_scope) =="
...
# to green because the step did the work.
...
# THE behavioral check, and the whole point of the step: an unprivileged
...
# 2>/dev/null on the ssh call itself: the pre-auth banner (step 9) lands on
# stderr and would otherwise print in the middle of the check list.
...
echo "== Step 31: guess cost (yescrypt cost factor + fail delay) =="
...
# login.defs (verified empirically on the node before writing the step).
...
# the step this same probe fails in ~2.5 s at worst (pam_unix's built-in
...
echo "== Step 32: root PATH integrity (CIS 6.2.8) =="
# The CI plants offenders in ALL THREE sources that set root's PATH on Debian,
# so every check below flips from red to green because the step did the work.
#
...
# UNFIXABLE one is dropped. This is also where the step's own bug was caught:
...
echo "== Step 35: SSH crypto policy =="
...
# "weak MAC accepted" while probing for this step). The weak-MAC probes
# force a ctr cipher on purpose: with an AEAD cipher (chacha20/GCM) OpenSSH
# skips MAC selection entirely, so a weak MAC only ever bites on ctr — that
# nuance is why the Ciphers pin alone would not close this door.
...
echo "== Step 36: legacy protocol purge (CIS 2.2/2.3) =="
...
# mirrors LEGACY_PROTOCOL_PACKAGES in harden.sh — both halves of every
# transitional pair, because purging `telnet` alone leaves inetutils-telnet
...
echo "== Step 38: account database hygiene =="
...
# Not every kernel exposes the JIT knob (this WSL one doesn't — measured):
# where it exists it must be 2; where it doesn't, the drop-in still carries
# the pin for kernels that do.
...
echo "== Step 41: time synchronization =="
...
# The honesty fork, measured while designing the step: timesyncd ships
...
# host). Assert the refusal is the CONDITION, not a crash: ConditionResult
# reads over systemd's private socket, no dbus needed (also measured).
if on_node systemd-detect-virt --container --quiet 2>/dev/null; then
  ...
  # Behavioural on a real machine: timesyncd touches this flag file on its
  # FIRST successful sync (it is what systemd-time-wait-sync waits for).
  synced=no
...
# umask step (027) would otherwise hide the probe from it.
...
echo "== Step 43: /var/tmp confinement =="
...
on_node cp /bin/true /var/tmp/db-harden-probe 2>/dev/null || true
if on_node /var/tmp/db-harden-probe >/dev/null 2>&1; then
...
on_node rm -f /var/tmp/db-harden-probe 2>/dev/null || true

echo "== Step 44: attack-surface services =="
# The CI installs avahi-daemon, cups (metapackage AND cups-daemon) and rpcbind
# on the node before hardening, so each of these flips red-to-green from real
...
# Behavioural: the three listeners are gone — nothing on mDNS (udp/5353),
# IPP (tcp/631) or the portmapper (tcp+udp/111).
expect_ok "nothing listens on mDNS, IPP or the portmapper (5353/631/111)" \
  bash -c "'! ss -lntu | grep -E \":(5353|631|111)[[:space:]]\"'"

echo "== Step 45: kernel attack surface =="
# The CI plants every interface open before hardening (io_uring allowed,
...
echo "== Step 47: per-session process limits =="
...
expect_line "the admin session's soft process limit is 4096 (effective)" '^4096$' \
  bash -c "'ulimit -Su'"
# Behavioural: the cap is a HARD limit - the session cannot raise it (a soft
...
echo "== Step 48: console reboot surface =="
...
# drop-in that closes it (config-effective, the sysctl-drop-in precedent).
...
echo "== Step 49: egress filtering =="
# Step 5 proved who may reach the box. This proves what the box may reach -
# the door a reverse shell, a dropper or an exfil walks out of. The policy is
# read from ufw itself, then the behaviour is measured from the node.
...
# retry whole waves rather than assuming one burst is enough.
banned=no
for wave in 1 2 3; do
  ...
  for _ in $(seq 1 8); do
    if docker exec db-harden-node fail2ban-client status sshd 2>/dev/null \
